...
const VS = 0.22;       // metres of arrow per m/s
const X: [number, number] = [-2.2, 2.2];
...
export default function AimTheBead({ id, prompt, pulses, bead, explanation }: AimTheBeadProps) {
  ...
  return (
    <SceneCard id={id} prompt={prompt}
      ...
      <Stage x={X} y={[-0.92, 0.92]} height={280} equal
        ...
        {(s) => {
          api.current = s;
          return <>
            <line x1={s.sx(bead)} x2={s.sx(bead)} y1={s.sy(-0.88)} y2={s.sy(-0.8)} stroke={C.position} strokeWidth={2} />
            ...
            {pulses.map((p, i) => {
              // Off to the trailing side of the crest, so the bead's own arrow has room.
              const yTop = p.amp > 0 ? p.amp + 0.3 : p.amp - 0.3, x0 = p.centre - Math.sign(p.speed) * 1.1;
              return <Arrow key={`v${i}`} s={s} from={[x0, yTop]} to={[x0 + p.speed * 0.25, yTop]} color={C.velocity} width={2.5}
