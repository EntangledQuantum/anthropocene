      ...
      <Stage x={[-3, 3]} y={[-1.5, 2.5]} height={330} equal
        ...
        {(s) => {
          ...
          return <>
            ...
            <text x={s.sx(base[0] + 0.75)} y={s.sy(base[1] + 0.12)} fontSize={14} fill={C.soft}>{deg.toFixed(1)}°</text>
            ...
            <Arrow s={s} from={c} to={add2(c, acc)} color={C.accel} width={4} dash="7 5" label={`a = ${r.accel.toFixed(2)} m/s²`} labelSide={-1} />
            ...
            <Arrow s={s} from={scale2(t, 0.35 * Math.sign(r.friction || 1))} to={add2(scale2(t, 0.35 * Math.sign(r.friction || 1)), scale2(t, r.friction * kF))} color={C.force} label={fk >= 0.005 ? 'friction' : undefined} />
