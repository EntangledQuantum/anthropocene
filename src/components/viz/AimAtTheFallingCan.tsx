...
export default function AimAtTheFallingCan({
  ...
  const fire = () => {
    ...
    const frame = (now: number) => {
      const i = Math.max(0, Math.min(iEnd, Math.floor(((now - t0) / 1000) * 0.5 / 0.002))); // half speed
      const b = r.ball[i], c = r.can[Math.min(i, r.can.length - 1)];
  ...
  return (
    <SceneCard id={id} prompt={prompt}
      ...
      <Stage x={[-1, can[0] + 2.6]} y={[-0.9, can[1] + 1.8]} height={340} equal ground
        ...
        {(s) => { sRef.current = s; const far = can[0] + 3; return <>
          {[0, 2, 4, 6, 8, 10, 12, 14].filter((m) => m <= can[0] + 2).map((m) => <text key={m} x={s.sx(m)} y={s.sy(0) + 20}
          ...
          {task.done && <text x={s.sx(-0.6)} y={s.sy(can[1] + 1.2)} fontSize={12} fill={C.faint}>aimed {straight.toFixed(1)}°, straight at the can</text>}
        </>; }}
      </Stage>
    </SceneCard>
  );
