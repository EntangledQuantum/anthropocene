    ...
    <SceneCard id={id} prompt={prompt}
      ...
      <Stage x={[-2.4, 2.4]} y={[-1.4, 1.4]} height={340} equal
        ...
        {(s) => <>
          ...
          <FluxTicks s={s} rods={tube} loop={loop} perNC={0.004} count={32} max={0.3} />
          ...
          <SleeveGrab s={s} loop={loop} centre={c} step={0.02} onMove={(p) => { setC([Math.min(2, Math.max(-2, p[0])), Math.min(1.2, Math.max(-1.2, p[1]))]); task.touch(); }} />
